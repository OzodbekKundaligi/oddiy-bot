...
def show_startups(message):
    ...
    bot.send_message(message.chat.id, "🌐 <b>Startaplar ro'yxati:</b>")
...
def show_startup_page(chat_id, page):
    ...
    markup.add(InlineKeyboardButton('🤝 Startupga qo\'shilish',
                               callback_data=f'join_startup_{startup["id"]}'))
...
def approve_join_request(call):
    try:
        ...
        cursor.execute('''
            SELECT sm.user_id, s.name, s.group_link 
